...
pub async fn mempool_worker<Pool>(mempool: Pool, bc: Blockchain<EthPrimitives>) -> eyre::Result<()>
where
        ...
        select! {
            tx_notification = tx_listener.recv() => {
                if let Some(tx_notification) = tx_notification {
                    ...
                    let update_msg: MessageMempoolDataUpdate<EthPrimitives> = MessageMempoolDataUpdate::new_with_source(NodeMempoolDataUpdate { tx_hash, mempool_tx: MempoolTx { tx: None, ..MempoolTx::default() } }, "exex".to_string());
