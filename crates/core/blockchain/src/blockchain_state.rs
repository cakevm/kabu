...
#[derive(Clone)]
pub struct BlockchainState<DB: Clone + Send + Sync + 'static, NP: NodePrimitives = EthPrimitives> {
    market_state: Arc<RwLock<MarketState<DB>>>,
    block_history_state: Arc<RwLock<BlockHistory<DB, NP>>>,
}
    ...
    > BlockchainState<DB, NP>
{
    pub fn new() -> Self {
        BlockchainState {
            ...
            block_history_state: Arc::new(RwLock::new(BlockHistory::<DB, NP>::new(10))),
        }
    }
    ...
    pub fn with_market_state(self, market_state: MarketState<DB>) -> BlockchainState<DB, NP> {
        BlockchainState { market_state: Arc::new(RwLock::new(market_state)), ..self.clone() }
    }
}

impl<DB: Clone + Send + Sync, NP: NodePrimitives> BlockchainState<DB, NP> {
    ...
    pub fn block_history(&self) -> Arc<RwLock<BlockHistory<DB, NP>>> {
