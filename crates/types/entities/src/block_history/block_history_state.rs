...
pub trait BlockHistoryState<N>
where
    N: NodePrimitives,
{
    fn apply_update(self, block_history_entry: &BlockHistoryEntry<N>, market_state_config: &MarketStateConfig) -> Self;
}

impl<N: NodePrimitives> BlockHistoryState<N> for KabuDB {
    fn apply_update(self, block_history_entry: &BlockHistoryEntry<N>, market_state_config: &MarketStateConfig) -> Self {
        let mut db = self;
