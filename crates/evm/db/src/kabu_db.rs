    ...
    Self: Sized + Send + Sync,
{
    ...
    pub read_only_db: Option<Arc<KabuDB>>,
    pub ext_db: Option<Arc<dyn DatabaseRef<Error = KabuDBError> + Send + Sync>>,
}

impl Debug for KabuDB {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("KabuDB").field("accounts", &self.accounts).finish()
    }
}

impl Default for KabuDB {
    fn default() -> Self {
        KabuDB::new().with_ext_db(EmptyDBTyped::<KabuDBError>::new())
    }
}
...
impl KabuDB {
    ...
    pub fn read_only_db(&self) -> Self {
        self.read_only_db.clone().map_or(KabuDB::empty(), |a| a.as_ref().clone())
    }
    ...
    where
        ExtDB: DatabaseRef<Error = KabuDBError> + Send + Sync + 'static,
        Self: Sized,
    {
        let ext_db = Arc::new(ext_db) as Arc<dyn DatabaseRef<Error = KabuDBError> + Send + Sync>;
    ...
    pub fn load_ro_rw_ext_account(&mut self, address: Address) -> Result<&mut FastDbAccount> {
        match self.accounts.entry(address) {
            ...
            Entry::Vacant(entry) => Ok(entry.insert(
                KabuDBHelper::get_or_fetch_basic(&self.read_only_db, &self.ext_db, address)
                    .unwrap_or_default()
    ...
    pub fn load_ro_rw_account(&mut self, address: Address) -> Result<&mut FastDbAccount> {
        match self.accounts.entry(address) {
            Entry::Occupied(entry) => Ok(entry.into_mut()),
            Entry::Vacant(entry) => Ok(entry.insert(
                KabuDBHelper::get_basic(&self.read_only_db, address)
                    .unwrap_or_default()
                    .map(|info| FastDbAccount { info, ..Default::default() })
                    .unwrap_or_else(FastDbAccount::new_not_existing),
            )),
    ...
    pub fn merge_all(self) -> KabuDB {
        ...
        KabuDB { read_only_db, ext_db: self.ext_db, ..Default::default() }
    }
...
impl DatabaseRef for KabuDB {
    ...
    fn code_by_hash_ref(&self, code_hash: B256) -> Result<Bytecode, Self::Error> {
        match self.contracts.get(&code_hash) {
            ...
            None => KabuDBHelper::get_code_by_hash(&self.read_only_db, code_hash),
        }
    }
            ...
            Some(acc_entry) => match acc_entry.storage.get(&slot) {
                ...
                None => {
                    if matches!(acc_entry.account_state, DBAccountState::StorageCleared | DBAccountState::NotExisting) {
                        ...
                        KabuDBHelper::get_or_fetch_storage(&self.read_only_db, &self.ext_db, address, slot)
                    }
                }
            },
            None => KabuDBHelper::get_or_fetch_storage(&self.read_only_db, &self.ext_db, address, slot),
        }
    }
    ...
    fn block_hash_ref(&self, number: BlockNumber) -> Result<B256, Self::Error> {
        match self.block_hashes.get(&number) {
            Some(entry) => Ok(*entry),
            None => KabuDBHelper::get_or_fetch_block_hash(&self.read_only_db, &self.ext_db, number),
        }
    }
}
...
impl Database for KabuDB {
    ...
    fn code_by_hash(&mut self, code_hash: B256) -> std::result::Result<Bytecode, Self::Error> {
        match self.contracts.entry(code_hash) {
            ...
            Entry::Vacant(entry) => {
                ...
                Ok(entry.insert(KabuDBHelper::get_code_by_hash(&self.read_only_db, code_hash)?).clone())
            }
        }
    }
    ...
    fn storage(&mut self, address: Address, slot: U256) -> std::result::Result<U256, Self::Error> {
        ...
        match self.accounts.entry(address) {
                ...
                match acc_entry.storage.entry(slot) {
                    ...
                    Entry::Vacant(entry) => {
                        if matches!(acc_entry.account_state, DBAccountState::StorageCleared | DBAccountState::NotExisting) {
                            ...
                            let slot = KabuDBHelper::get_or_fetch_storage(&self.read_only_db, &self.ext_db, address, slot)?;
                            entry.insert(slot);
                            Ok(slot)
                        }
                    }
            ...
            Entry::Vacant(acc_entry) => {
                let info = KabuDBHelper::get_or_fetch_basic(&self.read_only_db, &self.ext_db, address)?;
                let (account, value) = if info.is_some() {
                    let value = KabuDBHelper::get_or_fetch_storage(&self.read_only_db, &self.ext_db, address, slot)?;
    ...
    fn block_hash(&mut self, number: BlockNumber) -> std::result::Result<B256, Self::Error> {
        match self.block_hashes.entry(number) {
            Entry::Occupied(entry) => Ok(*entry.get()),
            Entry::Vacant(entry) => {
                let hash = KabuDBHelper::get_or_fetch_block_hash(&self.read_only_db, &self.ext_db, number)?;
                entry.insert(hash);
                Ok(hash)
            }
        }
...
mod test {
    ...
    fn test_new_with_provider() {
        ...
        rt.block_on(async move {
            ...
            let db = KabuDB::new_with_ro_db_and_provider(Some(db), provider.clone()).unwrap();
    ...
    fn test_new_with_ext_db() {
        ...
        rt.block_on(async move {
            ...
            let db = KabuDB::new_with_ext_db(db, ext_db);
    ...
    fn test_update_cell() {
        ...
        let mut new_state = KabuDB::new().with_ro_db(Some(init_state)).with_ext_db(EmptyDBTyped::<KabuDBError>::new());
        assert_eq!(new_state.accounts.len(), 0);
